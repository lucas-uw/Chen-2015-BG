...
******************************************************************************/
{
 ...
 /* for slope calculation*/
  ...
  int **pft_count;
  float **pft_frac;
  int wet_count,dry_count,found_wet_river_tibet;
              ...
              if(pft_data[ii_pft][jj_pft]>0 || glwd_data[ii_glwd][jj_glwd]!= nodata_glwd) {
                ...
                if(ii_river>=0 && jj_river>=0 && ii_river<nrows_river && jj_river<ncols_river) {
                  if (river_data[ii_river][jj_river] != nodata_river) {
                    if (river_data[ii_river][jj_river] != 10 && river_data[ii_river][jj_river] != 11 && river_data[ii_river][jj_river]!=12) { // Get rid of 10(25-50%wetland) and 11(50-100%wetland).20130123
                      found_wet_river = 1;
                    }
                    if (river_data[ii_river][jj_river] == 12) {
                      found_wet_river_tibet++;
                    }
                  }
...
//    fprintf(stdout,"lai calculation is finished\n");
    ...
    if(m==11){
      if(land_pix > 0) {
        wet_frac = ((float)wet_count+(float)found_wet_river_tibet*0.125)/(float)land_pix;
